@php
    ...
    $imgHeight = $isFlash ? '170px' : '210px';
    $minHeight = $isFlash ? '180px' : '220px';
@endphp
<div class="{{ $cardClass }} fade-up" data-slug="{{ $product->slug }}">
    <div class="pcard-img" style="min-height:{{ $minHeight }}">
        <a href="{{ route('products.show', $product->slug) }}" style="display:block;width:100%;">
            @if($product->image)
            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" loading="lazy" width="300" height="220" style="height:{{ $imgHeight }}">
            @else
            <img src="{{ asset('images/products/no-image.png') }}" alt="{{ $product->name }}" loading="lazy" width="300" height="220" style="height:{{ $imgHeight }}">
            @endif
        </a>
